...
#define HIDD_DEVICE_NAME "bt mouse (Lab4_3)"
...
const int16_t SMOOTH_FACTOR = 25;    // Smoothing factor for averaging mouse movements

// Function for simulating a mouse click
void bt_mouse_left_click(uint16_t id) {
    esp_hidd_send_mouse_value(id, HID_MOUSE_LEFT, 0, 0);
    esp_hidd_send_mouse_value(id, 0, 0, 0);
}

void bt_mouse_task(void *pvParameters)
{
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    uint8_t data[6];

    // To store the last processed value for smoothing
    int16_t last_x = 0;  
    int16_t last_y = 0;

    while(1) {
        if (sec_conn && read_register(0x0B, data, 6) == ESP_OK) {
            int16_t raw_x = (int16_t)((data[0] << 8) | data[1]);
            int16_t raw_y = (int16_t)((data[2] << 8) | data[3]);

            // Normalize the values
            int16_t normalized_x = raw_x / 250;
            int16_t normalized_y = raw_y / 125;

            // Smooth the mouse movement
            normalized_x = (last_x * (SMOOTH_FACTOR - 1) + normalized_x) / SMOOTH_FACTOR;
            normalized_y = (last_y * (SMOOTH_FACTOR - 1) + normalized_y) / SMOOTH_FACTOR;

            // printf("x: %d, y: %d\n", normalized_x, normalized_y);

            // Send the mouse movement data
            esp_hidd_send_mouse_value(hid_conn_id, 0, -normalized_x, -normalized_y);
            if (normalized_x == 0 && normalized_y == 0) {
                bt_mouse_left_click(hid_conn_id);
            }

            // Update last_x for smoothing
            last_x = normalized_x;
            last_y = normalized_y;
...
void app_main(void)
{
    ...
    xTaskCreate(&bt_mouse_task, "mouse_task", 2048, NULL, 5, NULL);
}
